...
class ClothesProduct(models.Model):
    ...
    category = models.CharField(max_length=50, choices=CATEGORY_CHOICES, default='ao')
    price = models.DecimalField(max_digits=14, decimal_places=2)
    stock = models.IntegerField(default=0)
    description = models.TextField(blank=True, null=True)
    image_url = models.CharField(max_length=500, blank=True, null=True)
    specs = models.JSONField(default=dict, blank=True)  # {color, size, material}
...
class InventoryImport(models.Model):
    product = models.ForeignKey(ClothesProduct, on_delete=models.CASCADE, related_name='imports')
    ...
    class Meta:
        db_table = 'clothes_inventory_imports'
        ordering = ['-created_at']
